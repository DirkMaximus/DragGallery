...
function Edit(props) {
	...
	function replaceImage(media) {
		...
		const att_img = getObjectCopy(props.attributes.images[index]);
		att_img.id = media.id;
		att_img.url = media.url;
		
		const att_images = getArrayCopy(props.attributes.images);
		att_images[index] = att_img;
		
		props.setAttributes({images: att_images});
    ...
    function imageDown(e) {
        ...
        window.dragGalleryEdit.setAttributeObject(props);
