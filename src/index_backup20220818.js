...
class Edit extends Component {
	...
	constructor(props) {
		...
		this.getNextAvailableID = this.getNextAvailableID.bind(this);
		this.getNextAvailableRow = this.getNextAvailableRow.bind(this);
    ...
    selectImage(media) {
        ...
        if(media.length) {//Array of images
            ...
            */
            let match = true;
            const offset = this.props.attributes.images.length - media.length;
            if(offset >= 0) {
                for(let i = this.props.attributes.images.length-(1+media.length); i < this.props.attributes.images-1; i++) {
                    if(this.props.attributes.images[i].id != media[i-offset].id) {
                        match = false;
                        break;
                    }
                }
        ...
        const images = this.props.attributes.images.concat(newImages);
    ...
    imageClick(e) {
		...
		jQuery(gallery).find('.image').removeClass('selected');
    ...
    getNextAvailableID() {
        ...
        let next = 1;
        for(let i = 0; i < this.props.attributes.images.length; i++) {
            if(this.props.attributes.images[i].imageID >= next) next = this.props.attributes.images[i].imageID+1;
        }
        return next;
    }
    ...
    getNextAvailableRow() {
        ...
        let next = 1;
        for(let i = 0; i < this.props.attributes.images.length; i++) {
            if(this.props.attributes.images[i].row >= next) next = this.props.attributes.images[i].row+1;
        }
        return next;
    }
    ...
    render() {
    	...
    	return(
	        <div className="wp-block-create-block-drag-gallery">
	        
	            <div className="drag-gallery">
	                <div className="wrapper">
	                    {self.props.attributes.images.map(function(item, i) {
	                        ...
	                        data-row={item.row}
	                        onMouseDown={self.imageDown}
	                        onClick={self.imageClick}>
	                             ...
	                             <Toolbar>
									...
									<ToolbarGroup>
										<ToolbarButton 
											...
											className="change"
											onClick={() => console.log('change image pressed')}
										/>
									</ToolbarGroup>
			                    </Toolbar>
